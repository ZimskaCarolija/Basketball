const {AddDay} = require('./time')
const GroupRound = (round, groups, groupSchedule,groupsName,exibitions,startDate) => {
    ...
    groupsName.forEach(groupName => {
        ...
        currentRoundSchedule.forEach(match => {
            Game(group[match[0]],group[match[1]],exibitions,startDate)
...
const Game = (obj1,obj2,exibitions,date)=>
{
    ...
    //Add exibitions
    exibitions[obj1.ISOCode].push({Date:date,Opponent:obj2.ISOCode,Result:obj1Score+"-"+obj2Score});
    exibitions[obj2.ISOCode].push({Date:date,Opponent:obj1.ISOCode,Result:obj2Score+"-"+obj1Score});
...
const EliminationStage=(hat,exibitions,date)=>
{
    let match = [];
    let temp = [];
    ...
    let semiFinals = [];
    let game1Res = Game(match[0][0],match[0][1],exibitions,date);
    Progress(semiFinals,match[0][0],match[0][1],game1Res)

    let game2Res = Game(match[1][0],match[1][1],exibitions,date);
    Progress(semiFinals,match[1][0],match[1][1],game2Res)

    date = AddDay(date);
    let game3Res = Game(match[2][0],match[2][1],exibitions,date);
    Progress(semiFinals,match[2][0],match[2][1],game3Res)

    let game4Res = Game(match[3][0],match[3][1],exibitions,date);
...
const SemiFinals=(match,exibitions,date)=>{
    console.log("Semi Finals");
    
    let finals=[];
    let place3=[];
    let game1Res = Game(match[0],match[2],exibitions,date);
    place3.push(Progress(finals,match[0],match[2],game1Res));
    date = AddDay(date)
    let game2Res = Game(match[1],match[3],exibitions,date);
    place3.push(Progress(finals,match[1],match[3],game2Res));
    return [finals,place3];
}
const Finals = (match,exibitions,date)=>{
    ...
    let game1Res = Game(place3[0],place3[1],exibitions,date);
    Progress(P3,place3[0],place3[1],game1Res);
    console.log("Finals");
    date = AddDay(date)
    let game2Res = Game(finals[0],finals[1],exibitions,date);
